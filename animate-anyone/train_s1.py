...
if __name__ == '__main__':
    ...
    stage_one_steps, stage_two_steps = 640000, 10000 # adjust stage_one_steps to be 30000*64=num_frames*batch_size*step
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            ...
            config={'num_gpus': accelerator.num_processes, 'num_frames': num_frames, 'learning_rate': learning_rate,
                        'batch_size': stage_one_batch_size, 'steps': stage_one_steps, 'mode': 'dataparallel_gpu', 'stage': 1}
        )
    ...
    if accelerator.is_main_process:
        pbar = tqdm(total=stage_one_steps)
    ...
    while stage_one_train:
        ...
        for step, data in enumerate(train_dataloader):
            ...
            accelerator.log({"stage_one_train_loss": avg_loss})
            ...
            global_step += accelerator.num_processes * stage_one_batch_size
            if accelerator.is_main_process:
                tqdm.write(f'step: {global_step} loss: {avg_loss}')
                pbar.update(accelerator.num_processes * stage_one_batch_size)
